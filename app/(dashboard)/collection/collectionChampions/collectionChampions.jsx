...
export default memo(function MainPage() {
  ...
  const { start, cancel, end } = useHoverIntent({ initialDelay: 700 });
  ...
  const { data: champions, isLoadingChampionsData } = useQuery({
    queryKey: ["champions"],
  ...
  const onHoverStart = (champion, position) => {
    start(() => {
      /*setToolTipPos(position)*/
      setHoveredChampion({ champion, position });
    });
    /*setToolTipPos({
            x: e.clientX,
            y: e.clientY
  ...
  const onHoverEnd = useCallback(() => {
    end(() => setHoveredChampion({ champion: null, position: null }));
    cancel();
  ...
  const groupedChampions = useMemo(() => {
    ...
    return result;
  }, [champions, searchKeys, inCollection, sortedBy, groupedBy]);
    ...
    <div className="collection-champions-wrapper">
      {!selectedChampion ? (
        <div
          ...
          style={{ pointerEvents: `${selectedChampion ? "none" : null}` }}
        >
      ...
      {!selectedChampion ? (
        <ToolTip
          delay={100}
          content={{
            championName: hoveredChampion?.champion?.name || null,
            masteryLevel: 1,
            masteryPoints: 0,
            maxSeasonRating: "N/D",
            startInfo: "A",
