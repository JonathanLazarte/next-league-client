...
const Pvp = dynamic(() => import('./PvpRoom/index.jsx'), { ssr: false })
const Explore = dynamic(() => import('./Training/explore.jsx'), { ssr: false })
*/
import './room.css';
  ...
  PVP: [
    {
      ...
      queues: [
        {
          name: 'Swiftplay',
          description: 'Authentic SR gameplay in a much shorter match. Play with friends of different skill levels without the fear of falling too far behind.',
        },
        {
          name: 'Ranked Solo/Duo',
          description: 'Crush your lane, dive into epic five-on-five team fights, and destroy the enemy nexus in League`s premier competitive mode.',
        },
        {
          name: 'Ranked Flex',
          description: 'Crush your lane, dive into epic five-on-five team fights, and destroy the enemy nexus in League`s premier competitive mode.',
...
export default memo(function ModeSelection({
  ...
  const handleCategoryChange = (category) => {
    playMenuClick()
    GAME_DATA[category] ? setCategorySelected(category) : console.log("categoria incorrecta")
  };
  // Si ya eligió modo → mostrar sala o exploración

    const PlaySelectionLayer = () => {
      return <section className="play-selection-modes">
          <header className="mode-selection-header">
          <div
            className={`subheader-item ${categorySelected === 'PVP' ? 'active-subheader-item' : ''}`}
          ...
          <div
            className={`subheader-item ${categorySelected === 'TRAINING' ? 'active-subheader-item' : ''}`}
  ...
  const isQueueSelected = userState === 'Ranked Solo/Duo' || userState === 'Intermedio' || userState === 'Ranked Flex' || userState === 'Swiftplay';
  ...
  return (
    ...
    >
    <div className={`background-engine ${ isQueueSelected ? 'in-room' : null }`}>
      <div className="bg-layer bg-lobby"></div>
      <div className="bg-layer bg-room"></div>
    </div>

      { !isQueueSelected && <PlaySelectionLayer/> }
      { (userState === 'Ranked Solo/Duo' || userState === 'Ranked Flex' || userState === 'Swiftplay') && 
      <Pvp
        socket={socket}
